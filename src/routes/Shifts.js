...
router.get("/", ...mw, async (req, res) => {
  try {
    ...
    res.json({ shifts });
  } catch (e) { res.status(500).json({ error: "Failed to fetch shifts" }); }
});
...
router.post("/", ...mw, ...canManage, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: "Failed to create shift" }); }
});
...
router.put("/:id", ...mw, ...canManage, async (req, res) => {
  try {
    ...
    res.json({ shift });
  } catch (e) { res.status(500).json({ error: "Failed to update shift" }); }
});
...
router.delete("/:id", ...mw, ...canManage, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: "Failed to delete shift" }); }
});
...
router.get("/my-shift", ...mw, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: "Failed to fetch your shift" }); }
});
...
router.post("/assign", ...mw, ...canManage, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: "Failed to assign shift" }); }
});
...
router.delete("/assignments/:id", ...mw, ...canManage, async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: "Failed to remove assignment" }); }
});
