...
router.get("/payroll-export", ...mw, canManage, async (req, res) => {
  try {
    const period = req.query.period || new Date().toISOString().slice(0, 7);
    const company = await Company.findById(req.user.company).select("payroll name");
    const settings = company?.payroll || {};
    ...
    timesheets.forEach(ts => {
      const id = ts.employee?._id?.toString();
      if (!id) return;
      ...
      const std = settings.standardHours || 160;
      const total = ts.totalHours || 0;
      empMap[id].regularHours += Math.min(total, std);
      empMap[id].overtimeHours += Math.max(0, total - std);
    });

    expenses.forEach(exp => {
      const id = exp.employee?._id?.toString();
      if (!id) return;
      if (!empMap[id]) empMap[id] = {
        name: exp.employee.name,
        employeeId: exp.employee.employeeId || "",
        department: "",
        regularHours: 0,
        overtimeHours: 0,
        expenseReimbursement: 0,
      };
      empMap[id].expenseReimbursement += exp.amount;
    });

    const rows = [
      ["Employee Name", "Employee ID", "Department", "Regular Hours", "Overtime Hours",
       `Overtime Rate (${settings.overtimeRate || 1.5}x)`, `Expense Reimbursement (${settings.currency || "GHS"})`, "Period"],
    ];

    Object.values(empMap).forEach(r => {
      rows.push([
        r.name, r.employeeId, r.department,
        r.regularHours, r.overtimeHours,
        settings.overtimeRate || 1.5,
        r.expenseReimbursement.toFixed(2),
        period,
      ]);
    });
    ...
    res.send(csv);
  } catch (e) { console.error(e); res.status(500).json({ error: "Failed to generate payroll export" }); }
});
