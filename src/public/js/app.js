...
content.innerHTML = ` <div class="page-header"><div><h2>Department Overview</h2></div></div> <div style="padding:16px 18px;background:#fffbeb;border:1.5px solid #fde68a;border-radius:10px;margin-bottom:18px;display:flex;gap:12px;align-items:center;"> <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#d97706" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> <div> <div style="font-weight:700;font-size:13px;color:#92400e;">No Department Assigned</div> <div style="font-size:12px;color:#b45309;margin-top:2px;">Your account has no department set. Please ask your institution admin to update your profile with your department name. Until then you will see all data in the institution.</div> </div> </div>`;
...
content.innerHTML = `<div class="page-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;"> <div><h2>Quizzes</h2><p>Your available quizzes and assessments</p></div> <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:0.9em;"> <input type="checkbox" ${showAll ? 'checked' : ''} onchange="renderStudentQuizzes(document.getElementById('main-content'), this.checked)"> Show past & upcoming </label> </div> <div class="card"> ${quizzes.length ?`
<table>
...
content.innerHTML = `<div class="page-header"><h2>Reports</h2><p>Download reports as PDF with one click</p></div> <div class="reports-grid">${cards}</div>`;
}
...
</div>`).join('')} <div id="gb-weights-err" style="display:none;color:#ef4444;font-size:12px;"></div> </div> <div style="padding:12px 20px;border-top:1px solid var(--border);display:flex;gap:8px;justify-content:flex-end;"> <button class="btn btn-secondary" onclick="document.getElementById('gb-weights-overlay').remove()">Cancel</button> <button class="btn btn-primary" onclick="saveWeights('${courseId}')">Save</button> </div> </div>`;
document.body.appendChild(ol);
}
...
return `<div style="display:grid;grid-template-columns:${daysToShow.map(()=>'1fr').join(' ')};gap:8px;margin-bottom:20px;"> ${daysToShow.map(d =>`
...
return ` <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px"> <div style="grid-column:1/-1"> <label style="font-size:11px;font-weight:700;text-transform:uppercase;color:#6b7280;display:block;margin-bottom:4px">Title *</label> <input id="${prefix}-title" placeholder="Goal title" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;font-size:13px"> </div> <div> <label style="font-size:11px;font-weight:700;text-transform:uppercase;color:#6b7280;display:block;margin-bottom:4px">Category</label> <select id="${prefix}-cat" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;font-size:13px"> <option value="kpi">KPI</option><option value="personal">Personal</option><option value="team">Team</option><option value="learning">Learning</option> </select> </div> <div> <label style="font-size:11px;font-weight:700;text-transform:uppercase;color:#6b7280;display:block;margin-bottom:4px">Period</label> <select id="${prefix}-period" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;font-size:13px"> <option value="monthly">Monthly</option><option value="quarterly" selected>Quarterly</option><option value="annual">Annual</option> </select> </div> <div> <label style="font-size:11px;font-weight:700;text-transform:uppercase;color:#6b7280;display:block;margin-bottom:4px">Target</label> <input id="${prefix}-target" type="number" placeholder="e.g. 100" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;font-size:13px"> </div> <div> <label style="font-size:11px;font-weight:700;text-transform:uppercase;color:#6b7280;display:block;margin-bottom:4px">Unit</label> <input id="${prefix}-unit" placeholder="e.g. calls, %" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;font-size:13px"> </div> <div style="grid-column:1/-1"> <label style="font-size:11px;font-weight:700;text-transform:uppercase;color:#6b7280;display:block;margin-bottom:4px">Due Date</label> <input id="${prefix}-due" type="date" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px;font-size:13px"> </div> </div>`;
}
