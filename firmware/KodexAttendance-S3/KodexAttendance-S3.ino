...
"if(!code){err.textContent='Please enter the attendance code shown on the device screen.';err.style.display='block';return;}"
...
"fetch('/attend',{method:'POST',headers:{'Content-Type':'application/json'},"
"body:JSON.stringify({indexNumber:idx,code:code,remark:true})})"
".then(function(r){return r.json().then(function(d){return{ok:r.ok,d:d};});})"
".then(function(r){"
