...
- **Mobile app**: Flutter — `dikly_flutter/` — **PAUSED (owner instruction, 2026-07-21). Do not build, run, or make changes to this directory until the owner says to resume.** Goal when resumed: every page matches the web app exactly, verified against the web code — no guessing. Builds on Flutter 3.44.6 stable; requires `dl.google.com` in the environment's network allowlist for Gradle/AGP artifact downloads.
...
1. **Flutter — PAUSED (owner instruction, 2026-07-21)** — Do not build, run, or make changes to `dikly_flutter/` until the owner explicitly says to resume. State when paused, for reference once resumed: bringing `dikly_flutter/` to exact parity with the web app, page by page, tracing every claim to real web/server code. Done so far: 5 dead API endpoints fixed, Expenses/Payroll removed (no-finance rule), 401→refresh-token session survival added, 2FA login gate added, meeting-room `dikly_auth` token handoff hardened; analyzer clean on Flutter 3.44.6. Remaining: screen-by-screen UI/behavior parity (web page inventory: 89 pages via `navigateTo` switch; Flutter: ~70 routes), then release APK. APK builds need `dl.google.com` allowed in the Claude Code environment network policy (maven.google.com 301s every artifact there); Gradle 8.9 itself is already seeded in the build cache, SHA-256-verified against the official checksum.
